...
read -p 'Please enter a folder name: ' folder_name &&  cd "$folder_name" #||echo "File does not exist" ; exit 0 #||mkdir "$folder_name" && cd "$folder_name" ## now why does it fail to find existing files if the fail condition is set?
